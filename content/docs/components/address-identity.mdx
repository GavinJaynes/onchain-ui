...
<InstallTabs command="shadcn add https://onchain-ui.dev/r/address-identity.json" />

<OpenInV0 name="address-identity-demo" />
...
<InstallTabs packages="viem sonner lucide-react" />
...
<Callout type="warn">
The default resolvers use each chain's public RPC endpoint with an automatic fallback to a second public endpoint, since some public RPCs aggressively rate limit. Fine for demos and prototypes; in production, pass your own viem clients through `resolverOptions`.
</Callout>
...
By default, resolution happens in the browser after mount, so the first paint shows the truncated address until the lookup lands. To render the resolved name from the first byte — no flash, no layout shift — resolve on the server and pass the result in. The resolvers only depend on viem, so they run in any server context.

Pass `resolveIdentity={false}` alongside the resolved values so the component skips client-side lookups entirely.
...
<Callout>
The session cache is per server process, so hot addresses resolve once per deployment instance rather than once per request. For high-traffic pages, wrap the loader call in your framework's request-level cache or a TanStack Query prefetch with a long `staleTime` — see [Data Fetching](/docs/recipes/data-fetching).
</Callout>
